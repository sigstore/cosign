...
func TestDoInitialize(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.wantStdOut == "" {
				if gotStdOut != "" {
					t.Errorf("expected empty stdout, got %q", gotStdOut)
				}
			} else {
				if !strings.Contains(gotStdOut, test.wantStdOut) {
					t.Errorf("expected stdout to contain %q, got %q", test.wantStdOut, gotStdOut)
				}
			}
			if test.wantStdErr == "" {
				if gotStdErr != "" {
				...
				if !strings.Contains(gotStdErr, test.wantStdErr) {
					t.Errorf("expected stderr to contain %q, got %q", test.wantStdErr, gotStdErr)
				}
			}
			...
			for _, f := range test.wantFiles {
				path := filepath.Join(tufCache, mirrorDir, f)
				if _, err := os.Stat(path); os.IsNotExist(err) {
					t.Errorf("expected file %s to exist, but it does not", path)
				}
			}
			remotePath := filepath.Join(tufCache, "remote.json")
			if _, err := os.Stat(remotePath); os.IsNotExist(err) {
				t.Errorf("expected file %s to exist, but it does not", remotePath)
			}
		})
	}
}
