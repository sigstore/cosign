...
func doInitialize(ctx context.Context, root, mirror string) error {
	return runInitialize(ctx, root, mirror, "", true)
}

func doInitializeWithRootChecksum(ctx context.Context, root, mirror, rootChecksum string) error {
	return runInitialize(ctx, root, mirror, rootChecksum, false)
}

func doInitializeStaging(ctx context.Context) error {
	return runInitialize(ctx, "", tuf.StagingMirror, "", true)
}

func runInitialize(ctx context.Context, root, mirror, rootChecksum string, forceSkipChecksumValidation bool) error {
	...
	if root != "" {
		if !forceSkipChecksumValidation {
			if rootChecksum == "" && (strings.HasPrefix(root, "http://") || strings.HasPrefix(root, "https://")) {
				fmt.Fprintln(os.Stderr, RootWithoutChecksumDeprecation)
			}
		}
