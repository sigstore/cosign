...
func signDigestBundle(ctx context.Context, digest name.Digest, ko options.KeyOpts, signOpts options.SignOptions) error {
	...
	if ko.SigningConfig != nil {
		return signcommon.WriteNewBundleWithSigningConfig(ctx, ko, signOpts.Cert, signOpts.CertChain, payload, digest, types.CosignSignPredicateType, signOpts.OutputBundle, signOpts.Upload, ko.SigningConfig, ko.TrustedMaterial, ociremoteOpts...)
	}
	...
	return signcommon.WriteBundle(ctx, bundleComponents.SV, bundleComponents.RekorEntry, payload, bundleComponents.SignedPayload, bundleComponents.SignerBytes, bundleComponents.TimestampBytes, digest, types.CosignSignPredicateType, signOpts.OutputBundle, signOpts.Upload, ociremoteOpts...)
}
