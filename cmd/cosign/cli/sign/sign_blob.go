...
func SignBlobCmd(ctx context.Context, ro *options.RootOptions, ko options.KeyOpts, payloadPath, certPath, certChainPath string, b64 bool, outputSignature string, outputCertificate string, tlogUpload bool) ([]byte, error) {
	...
	// If a certificate is provided, clear the idToken so that SignData uses
	// that certificate instead of fetching a new one from Fulcio.
	certIdToken := idToken
	if len(certBytes) > 0 {
		certIdToken = ""
	}
	...
	bundleBytes, err := cbundle.SignData(ctx, content, keypair, certIdToken, certBytes, ko.SigningConfig, ko.TrustedMaterial, signOpts)
	if err != nil {
