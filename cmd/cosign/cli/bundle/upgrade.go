...
func (c *UpgradeCmd) Exec(ctx context.Context) error {
	...
	rekorClient, err := rekorclient.GetRekorClient(c.RekorURL)
	if err != nil {
...
func detectFormat(data []byte) error {
	...
	if _, hasBase64Signature := m["base64Signature"]; hasBase64Signature {
		return fmt.Errorf("cannot upgrade legacy bundles; use `cosign bundle create` to create a new bundle from your old bundle and artifact")
	}

	return fmt.Errorf("unrecognized bundle format")
}

func upgradeBundle(ctx context.Context, data []byte, rekorClient *rekorgenclient.Rekor) ([]byte, error) {
