...
func NewSigner(ctx context.Context, idToken, oidcIssuer, oidcClientID, oidcClientSecret string, oidcCallbackPort int, fClient api.Client) (*fulcio.Signer, error) {
	fs, err := fulcio.NewSigner(ctx, idToken, oidcIssuer, oidcClientID, oidcClientSecret, oidcCallbackPort, fClient)
	if err != nil {
		return nil, err
	}
