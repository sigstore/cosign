...
```
  ...
  -r, --recursive                                                                                if a multi-arch image is specified, additionally sign each discrete image
      ...
      --sign-container-identity stringArray                                                      manually set the .critical.docker-reference field for the signed identity, which is useful when image proxies are being used where the pull reference should match the signature
